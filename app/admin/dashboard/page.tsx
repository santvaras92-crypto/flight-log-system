...
export default async function AdminDashboardPage({ searchParams }: { searchParams?: { page?: string; pageSize?: string } }) {
  ...
  const [users, aircraft, flights, allFlights, submissions, components, transactions, totalFlights] = await Promise.all([
    ...
    prisma.flight.findMany({ orderBy: { fecha: "desc" } }), // All flights for Active Pilots calculation
  ...
  const data = {
    ...
    allFlights: allFlights.map(f => ({ id: f.id, fecha: f.fecha, pilotoId: f.pilotoId, clienteCodigo: (f as any).clienteCodigo || null, diff_hobbs: Number(f.diff_hobbs), costo: Number(f.costo) })), // Lightweight for Active Pilots
